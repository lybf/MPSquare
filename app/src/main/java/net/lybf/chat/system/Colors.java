package net.lybf.chat.system;

public class Colors
{
    public static final int primary=0xFF3F51B5;
    public static final int primary_dark=0xFF303F9F;
    public static final int primary_light=0xFFC5CAE9;
    public static final int accent=0xFF448AFF;
    public static final int primary_text=0xFF212121;
    public static final int secondary_text=0xFF757575;
    public static final int icons=0xFFFFFFFF;
    public static final int divider=0xFFBDBDBD;
    
    public static final int white=0xFFFFFFFF;
    public static final int black=0xFF212021;
    public static final int gray=0xff444444;
    public static final int black2=0xFF030706;
    public static final int orange=0xFF9900;
    public static final int green=0x009900;
    public static final int blue=0x000099;
    public static final int transparent=0x00000000;
    public static final int border_bkg=0xFFFFFF;
    public static final int border=0x000000;
    public static final int text=0x000000;
    public static final int main_bkg=0xE3DBCF;
    public static final int button=0xFFFFFF;
    public static final int button_disable=0x9C8E8E;
    public static final int button_press=0xF1F1F1;
    public static final int button_text=0x000000;

}
